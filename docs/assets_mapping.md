...
| 게임 배경 | 10 | `public/assets/backgrounds/` |
| 폰트 | 1 | `public/fonts/` |
| **합계** | **34개 파일** | |
...
`bg-1.jpg` ~ `bg-10.jpg` → `GAME_BGS` 배열, 인덱스 0~9.

**선택 규칙**:
- 솔로 엔드리스: 레벨의 일의 자리 매핑 — `GAME_BGS[(level - 1) % GAME_BGS.length]`
  - 레벨 1, 11, 21... → `bg-1`
  - 레벨 10, 20, 30... → `bg-10`
- PvP: 게임 시작 시 1회 랜덤 — `GAME_BGS[Math.floor(Math.random() * GAME_BGS.length)]`

**헬퍼 함수** (`src/game/backgrounds.ts`):

```ts
import { GAME_BGS } from '@/assets';

export function getBackgroundForLevel(level: number): string {
  const idx = (level - 1) % GAME_BGS.length;
  return GAME_BGS[idx];
}
...
| 파일명 | 설명 |
|---|---|
| `moneygraphy.woff2` | 우리은행 머니그라피 픽셀 폰트 (둥글둥글한 한글) |
...
@font-face {
  ...
  src: url('/fonts/moneygraphy.woff2') format('woff2');
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}
...
@theme {
  --font-display: 'Moneygraphy', 'Pretendard', 'Apple SD Gothic Neo', system-ui, sans-serif;
}
```
...
└─ backgrounds.ts    # MAIN_HERO, TITLE_LOGO, PAGE_BGS, GAME_BGS
```
...
export { MAIN_HERO, TITLE_LOGO, PAGE_BGS, GAME_BGS } from './backgrounds';
```

...
- **v2** (2026-05-15): 신 등급(B/A/S/S+), 페어 시스템, sub-grade 확률 보정, 적용 범위 분기, S+ 게임 변형.
  - N 등급 → B로 명칭 변경
  - 기존 옷 3종(sprout/wings/angry) 폐기 또는 새 등급 정책에 맞춰 재정의 (사이클 2.5 시점엔 메타 미등록)
  - 단일 옷 명명 `cloth-{id}-*.png` → `chi-{id}-*.png` / `cat-{id}-*.png` (캐릭터 prefix 일관화)
  - `CLOTHES_BY_GRADE` 정적 객체 → `clothesByGrade(grade)` 헬퍼 함수
  - v2 작성 시점에 실제 옷 항목 0개
- **v2.1** (2026-05-20): §13 가챠 정책 정식화, §10.3 도감 모달 신설.
  - 가챠 비용 15코인 확정 (v1 데모 1코인, v2 미정)
  - 중복 환불 등급별 차등 (B:1 / A:3 / S:5)
  - S+ 단발 규칙 도입 (보유 ID 기반 동적 풀 필터링)
  - 도감 모달을 옷장 메인에서 분리 — `GridCard` 재사용
...
3. **보유 그리드** — 화면 하단, **보유한 옷만 표시** (오브젝트 아이콘)
   - 빈 상태(보유 0): 가챠 유도 안내
4. **도감 모달** — 그리드 영역의 "도감" 버튼으로 진입
5. **가챠 모달** — 가챠 버튼 클릭 → 결과 모달 (당첨 옷 풀바디 + 등급)

### 10.2 보유 그리드 카드 표시

- 보유 옷만 카드로 표시 (미보유는 메인 화면에 노출 X)
- 풀컬러 오브젝트 + 등급 배지
- 현재 장착 옷: 강조 테두리
- 페어 옷: 카드에 페어 표시 아이콘
...
수집 진척 확인용. 메인 옷장에서 분리해 인지 부담을 줄이고, 가챠 동기를 보강한다.

- **진입**: 그리드 영역의 "도감" 버튼
- **레이아웃**: 등급별 탭 (B / A / S / S+)
  - 탭 헤더에 진척 카운터 표시 (예: `B  8/12`)
  - 전체 진척은 모달 상단 또는 별도 슬롯 (사이클 W 결정)
- **카드 표시**:
  - 보유 옷: 풀컬러 + 이름 + 등급 (보유 그리드와 동일 카드)
  - 미보유 옷: 그레이스케일 또는 ??? (정확한 표기는 §12 미결정 항목)
- **상호작용**:
  - 보유 옷 카드 클릭 → 장착/해제 (보유 그리드와 동일)
  - 미보유 옷 카드 클릭 → 비활성 (또는 등급별 힌트 노출 — 사이클 W 결정)
- **컴포넌트 재사용**: `GridCard`(A2b-2)를 보유/미보유 분기 그대로 사용. 호출 측에서 필터만 다르게.
...
| 사이클                | 옷장 관련 작업                                                               |
| --------------------- | ---------------------------------------------------------------------------- |
| **2.5** (완료)        | 등급 정책, 메타 인터페이스, 경로 헬퍼, 시스템/워크플로우 문서                |
| **B** (진행 중)       | Chihuahua/Cat 컴포넌트에 `equippedSrc?: string` prop 정의. 옷 로직은 안 만짐 |
...
| 도감 미보유 카드 클릭 시 힌트 노출 여부       | 사이클 W              |
| 도감 진척 카운터 위치 (탭 헤더 vs 모달 상단)  | 사이클 W              |
| 보유 0 상태 안내 문구                         | 사이클 W              |
| S+ 풀 비었을 때 처리 (섹션 13.3 참조)         | 사이클 W              |
